...
--------------------------------------------------------------------------------
local textclock = {}
textclock.widget = redflat.widget.textclock({ timeformat = "%H:%M:%S", dateformat = "%b  %d  %a", timeout = 1 })
...
awful.screen.connect_for_each_screen(
	function(s)
		...
		-- tags
		awful.tag({ "Brow", "Mail", "Text", "Game"}, s, { al[1], al[1], al[1], al[1]})
		...
		s.panel:setup {
			...
			{ -- right widgets
				...
				separator,
				env.wrapper(volumecfg.widget, "volume"), --volume.buttons),
				separator,
